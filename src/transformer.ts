...
const fixLiteralString = (node:any):any => {
  return (
    node.kind === ts.SyntaxKind.JsxText && typeof node?.text === "string" ?
...
const transformForNode:JsxTransformation = (node, program, ctx) => {
  ...
  if (functionLoopBody) {
    const func = Option.of(functionLoopBody.getChildAt(1))
      .map(func => func.kind === ts.SyntaxKind.ParenthesizedExpression ?
        (
          func as any
        ).expression ?? func :
        func)
    ...
    if (ts.isArrowFunction(func) || ts.isFunctionExpression(func)) {
      ...
      return resolveExpression(node.parent, result)
